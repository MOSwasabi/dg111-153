...
**คำตอบ: value กับ hp เป็นคนละตัวเเปรกัน ถึงเเม้จะเขียนทับ value ไปเเล้ว**

### 2.2 `lerp` — Local Scope

**คำถาม:** ทำไม `main()` ถึงมองไม่เห็นตัวแปร `result` ทั้งที่มันถูกประกาศไว้ในไฟล์เดียวกัน?

**คำตอบ:  result อยู่ในห้องของ Lerp ทำให้ main มองไม่เห็น result ถึงเเม้จะอยู่ไฟเดียวกัน**

เเต่ result สามารถส่งค่าออกมาให้ main ได้
...
**คำตอบ: dicemin เเละ main เป็นคนละตัวเเปรกัน ถึงจะเเก้ตัว main เป็น 100 เเล้วเเต่ตัว dicemin ก็ยังคงเป็น 1 เลยกลายเป็น 101**
...
**คำตอบ: score ส่งต่าให้ cerrent ถึงเเก้ค่า cerrent ไปก็ยังไม่เปลี่ยนเเปลง เพราะ  cerrent รับค่ามาจาก score**
